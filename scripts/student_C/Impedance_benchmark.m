...
sym_targets = {'Vc_d0', 'Vc_q0', 'beta_i', 'beta_v', 'cf', 'cs', 'delta0', ...
               ...
               'i1_d', 'i1_q', 'Vc_d', 'Vc_q', 'i2_d', 'i2_q', 'Vcf_d', 'Vcf_q'
              }; 
...
Y_dd = sys_full(1, 3); % D-axis Admittance (Delta i2_D / Delta Vg_D)
Y_qq = sys_full(2, 4); % Q-axis Admittance (Delta i2_Q / Delta Vg_Q)
Y_dq = sys_full(1, 4); % Cross-coupling (Delta i1_D / Delta Vg_Q)
Y_qd = sys_full(2, 3); % Cross-coupling (Delta i1_Q / Delta Vg_D)
%% Reading CSV data
csvData = readtable('LCL_Simple_IBR.csv', 'NumHeaderLines', 1);
...
hold on
semilogx(csv_f, csv_dd_db, 'ro--', 'LineWidth', 1.2, 'MarkerSize', 4); % CSV points
hold off
grid on; 
ylabel('Magnitude (dB)'); 
xlabel('Frequency (Hz)');
...
hold on
semilogx(csv_f, csv_dq_db, 'ko--', 'LineWidth', 1.2, 'MarkerSize', 4); % CSV points
hold off
grid on; 
ylabel('Magnitude (dB)'); 
xlabel('Frequency (Hz)');
...
hold on
semilogx(csv_f, csv_qd_db, 'ko--', 'LineWidth', 1.2, 'MarkerSize', 4); % CSV points
hold off
grid on; 
ylabel('Magnitude (dB)'); 
xlabel('Frequency (Hz)');
...
hold on
semilogx(csv_f, csv_qq_db, 'bo--', 'LineWidth', 1.2, 'MarkerSize', 4); % CSV points
hold off
grid on; 
ylabel('Magnitude (dB)'); 
xlabel('Frequency (Hz)');
...
% colors = lines(length(w_dev_test_values)); % Generate distinct colors for the loop
% 
% % Frequencies for plotting
% frequencies_Hz = logspace(-3, 6, 1000); % Reduced points from 100,000 for loop performance
% frequencies_rad = 2 * pi * frequencies_Hz;
% 
% % Create the 2x2 Figure
% figure('Color', 'w', 'Position', [100, 100, 1000, 700]); 
% sgtitle('Analytical GFMI Admittance vs w_{dev}', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 16);
% 
% % Prepare subplots and plot the static CSV baseline first
% for i = 1:4
%     subplot(2,2,i);
%     hold on; grid on;
%     xlim([1, 10000]); ylim([-60, 60]);
%     xlabel('Frequency (Hz)'); ylabel('Magnitude (dB)');
% end
% 
% % Plot CSV Data (Baseline - Black dashed lines)
% subplot(2,2,1); semilogx(csv_f, csv_dd_db, 'ko--', 'LineWidth', 1.2, 'MarkerSize', 4, 'DisplayName', 'CSV Data'); title('d-d Axis Admittance (Y_{dd})');
% subplot(2,2,2); semilogx(csv_f, csv_dq_db, 'ko--', 'LineWidth', 1.2, 'MarkerSize', 4, 'DisplayName', 'CSV Data'); title('d-q Cross-Coupling (Y_{dq})');
% subplot(2,2,3); semilogx(csv_f, csv_qd_db, 'ko--', 'LineWidth', 1.2, 'MarkerSize', 4, 'DisplayName', 'CSV Data'); title('q-d Cross-Coupling (Y_{qd})');
% subplot(2,2,4); semilogx(csv_f, csv_qq_db, 'ko--', 'LineWidth', 1.2, 'MarkerSize', 4, 'DisplayName', 'CSV Data'); title('q-q Axis Admittance (Y_{qq})');
% 
% % --- Loop Through w_dev Values ---
% for k = 1:length(w_dev_test_values)
%     current_w_dev = w_dev_test_values(k);
% 
%     % 1. Update ONLY the w_dev value in the substitution array (Index 34)
%     num_values{34} = current_w_dev; 
% 
%     % 2. Substitute into matrices
%     A_num = double(subs(A_sys0, sym_targets, num_values));
%     B_num = double(subs(B_sys0, sym_targets, num_values));
%     C_num = double(subs(C_sys0, sym_targets, num_values));
%     D_num = double(subs(D_sys0, sym_targets, num_values));
% 
%     % 3. Create State-Space Model
%     sys_full = ss(A_num, B_num, C_num, D_num);
% 
%     % 4. Extract Admittance Transfer Functions
%     Y_dd = sys_full(1, 3);
%     Y_qq = sys_full(2, 4);
%     Y_dq = sys_full(1, 4);
%     Y_qd = sys_full(2, 3);
% 
%     % 5. Get Bode magnitudes
%     mag_dd = 20*log10(squeeze(bode(Y_dd, frequencies_rad)));
%     mag_dq = 20*log10(squeeze(bode(Y_dq, frequencies_rad)));
%     mag_qd = 20*log10(squeeze(bode(Y_qd, frequencies_rad)));
%     mag_qq = 20*log10(squeeze(bode(Y_qq, frequencies_rad)));
% 
%     % 6. Plot onto the existing subplots
%     plot_label = sprintf('Analytical w_{dev} = %.2f', current_w_dev);
% 
%     subplot(2,2,1); semilogx(frequencies_Hz, mag_dd, 'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', plot_label);
%     subplot(2,2,2); semilogx(frequencies_Hz, mag_dq, 'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', plot_label);
%     subplot(2,2,3); semilogx(frequencies_Hz, mag_qd, 'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', plot_label);
%     subplot(2,2,4); semilogx(frequencies_Hz, mag_qq, 'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', plot_label);
% end
% 
...
% set(gca, 'XScale', 'log')
% subplot(2,2,4); legend('Location', 'best');
% set(gca, 'XScale', 'log')
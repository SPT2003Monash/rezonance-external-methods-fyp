...
Ig_phasor = (Vc_phasor - Vg_phasor) / Zg;

% Store operating point values
delta0 = Angle_rad;   

Vg_D0 = real(Vg_phasor);
Vg_Q0 = imag(Vg_phasor);
...
Ig_dq0 = Ig_phasor * exp(-1j * Angle_rad) * sqrt(2);
ig_d0 = real(Ig_dq0);
ig_q0 = imag(Ig_dq0);
%% Closed Loop Substitution
% 1. List ALL symbolic targets for the FULL system
sym_targets = {'Vc_d0', 'Vc_q0', 'beta_i', 'beta_v', 'cf', 'delta0', ...
               'ig_d0', 'ig_q0', 'k_ii', 'k_pi', 'k_iv', 'k_pv', ...
               'lf1', 'lg', 'rd', 'rf1', 'rg', 'w', 'wn','td', ...
               'mp', 'wc_p', 'V_set', 'nq', 'wc', 'R_vir', 'X_vir', ... 
               'Vg_D0', 'Vg_Q0'}; 

% 2. List the exact workspace variables to substitute 
num_values  = {Vc_d0, Vc_q0, beta_i, beta_v, cf, delta0, ...
               ig_d0, ig_q0, k_ii, k_pi, k_iv, k_pv, ...
               lf1, lg, rd, rf1, rg, w, wn, td, ...
...
%% Reading CSV data
csvData = readtable('Inverter(Single)_singular_ibr.csv', 'NumHeaderLines', 1);
...
frequencies_Hz = logspace(0, 4, 1000); % Sweep from 1 Hz to 1000 Hz
